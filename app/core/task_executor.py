...
class TaskExecutor:
        ...
        try:
            ...
            async with self.session.post(task.webhook_url, json=payload) as response:
                if response.status == 200 or response.status == 204:
                    # Log success
                    self._log_task_execution(task, retry_count, "success", "Task executed successfully")
                    return True
                else:
                    # Log failure
    ...
    async def execute_task_with_retry(self, task: Task) -> bool:
        ...
        while retry_count <= task.max_retry and not success:
            ...
            if not success and retry_count < task.max_retry:
                ...
                logger.info(f"Task {task.id} failed, retrying in {wait_time} seconds... (retry {retry_count + 1}/{task.max_retry})")
    ...
    def _log_task_execution(self, task: Task, retry_count: int, status: str, message: str):
        """
