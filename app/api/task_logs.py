...
from app.schemas.task_log import TaskLogCreate, TaskLogUpdate, TaskLog as TaskLogSchema, TaskLogListResponse
...
@router.get("/{task_log_id}", response_model=TaskLogSchema, dependencies=[Depends(verify_token)])
...
@router.put("/{task_log_id}", response_model=TaskLogSchema, dependencies=[Depends(verify_token)])
def update_task_log(task_log_id: UUID, task_log: TaskLogUpdate, db: Session = Depends(get_db)):
    db_task_log = db.query(TaskLog).filter(TaskLog.id == task_log_id).first()
    if db_task_log is None:
        raise HTTPException(status_code=404, detail="Task log not found")
...
@router.get("/", response_model=TaskLogListResponse, dependencies=[Depends(verify_token)])
def list_task_logs(
    skip: int = 0, 
    limit: int = 100, 
...
@router.get("/task/{task_id}", response_model=TaskLogListResponse, dependencies=[Depends(verify_token)])
