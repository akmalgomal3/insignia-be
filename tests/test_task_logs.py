...
def test_get_task_log(auth_headers, sample_task):
    ...
    create_response = client.post("/task-logs/", json=task_log_data, headers=auth_headers)
...
def test_update_task_log(auth_headers, sample_task):
    ...
    update_response = client.put(f"/task-logs/{task_log_id}", json=update_data, headers=auth_headers)
...
def test_list_task_logs_with_task_id_filter(auth_headers, sample_task, sample_task_logs):
    # Test filtering by task_id
    response = client.get(f"/task-logs/?task_id={sample_task['id']}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert "task_logs" in data
    assert "total" in data
...
def test_list_task_logs_by_task_with_status_filter(auth_headers, sample_task, sample_task_logs):
    # Test listing task logs by task with status filter
    response = client.get(f"/task-logs/task/{sample_task['id']}?status=success", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert "task_logs" in data
    assert "total" in data
